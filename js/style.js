let list_Home=$(".list_Home").outerWidth();
...
$('#ToggleBtn').click(function(){
     ...
     $('#ToggleBtn   ').animate({marginLeft:`${list_Home}`},1000)
})
$('#closebtn').click(function(){
    $('.optionBox').animate({left:`-${list_Home}`},1000);
    $('#ToggleBtn ').animate({marginLeft:`0px`},1000)
})
$(window).scroll( function(){
    let Wscroll=$(window).scrollTop();
    if(Wscroll>about+10){
    $(".list_Home").css("backgroundColor","rgba(0, 0, 0, 0.3)");
 }
 else{
    $(".list_Home").css("backgroundColor","black");
 }
